      ...
      <Box mb={8}>
        {this.state.time >= start && end >= this.state.time ? (
          <Stack align="center" spacing={4}>
            <LotteryForm
              countdown={<CountDown now={this.state.time} future={end} />}
              lottery={lottery}
              currentEntryValues={entry && entry.entryMetadata}
              onSubmit={(data, actions) =>
                enterLottery(data, actions, toast, setEntry)
              }
            />
            <Stack
              w="100%"
            ...
            >
              <Text m={2} fontSize="lg">
                Special Interest Co-op Applications:
              </Text>
              <ChakraLink
                color="teal.500"
                isExternal
                href="https://forms.gle/rLUrToLn6bnJPjpGA"
              >
                Third World Co-op
              </ChakraLink>
              <ChakraLink
                color="teal.500"
                isExternal
                href="https://forms.gle/gcgWANdqBxtcdGDr6"
              >
                Third World Social Justice
              </ChakraLink>
            ...
            <Stack w="100%" spacing={2} p={5} textAlign="left">
              <Text noOfLines={2} flexWrap="wrap">
                You may change your entry after submitting up until the close of
                the lottery. After that, submissions are final.
              </Text>
              <Text flexWrap="wrap">
                If for some reason you are unable to submit with this page,
                please use{" "}
                <ChakraLink
                  isExternal
                  color="teal.500"
                  href="https://forms.gle/HSiLbm2Bon6d41XQ7"
                >
          ...
          <Text m={2} fontSize="lg">
            The lottery begins in{" "}
            <CountDown now={this.state.time} future={start} />
          </Text>
        )}
      </Box>
    );
  ...
  return (
    <Stack w="100%" mt={2} p={2} spacing={4} align="center">
      ...
      >
        ...
        <OrderedList my={4}>
          {entryMetadata.preferences &&
            entryMetadata.preferences.map((pref) => (
              <ListItem fontWeight="thin" key={pref}>
                {pref}
              </ListItem>
            ))}
        </OrderedList>
      ...
      <Container>
        ...
        >
          <Wall
            ...
            caught={
              <Center minH="calc(100vh - 5rem)">
                <Heading mx={4} fontWeight="thin" color="gray.500">
                  Please{" "}
                  <LoginButton
                    fontSize="1.6rem"
                    icon={true}
